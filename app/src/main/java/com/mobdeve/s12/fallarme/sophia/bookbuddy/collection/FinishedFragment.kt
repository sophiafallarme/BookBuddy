...
//            bookAdapter.updateBooks(books)
        ...
        bookAdapter.setOnItemClickListener(object : FinishedAdapter.OnItemClickListener {
            override fun onItemClick(book: Book) {
                Log.d("Finished Fragment", "Clicked book: ${book.title}")
    ...
    private fun refreshBooksList() {
        val books = myDbHelper.getBooksByAccountId(accountId).filter { it.status == "Finished" }
        Log.d("Finished Fragment", "Finished books refreshed: ${books.size}")
        bookAdapter.updateBooks(books)
    }
